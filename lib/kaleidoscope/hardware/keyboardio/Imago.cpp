...
ATMEGA_KEYSCANNER_IMPLEMENTATION(keyboardio::Imago);
...
void ImagoLEDDriver::ledDriverUnlockRegister(void) {
  twiSend(LED_DRIVER_ADDR, CMD_WRITE_ENABLE, WRITE_ENABLE_ONCE); //unlock
}

void ImagoLEDDriver::ledDriverSelectRegister(uint8_t page) {
...
void ImagoLEDDriver::ledDriverSetAllPwmTo(uint8_t step) {
